...
logging.basicConfig(format='%(asctime)s - %(levelname)s: %(message)s',
                    filename=log_path, level=logging.WARN, encoding='utf-8')
...
class Decode():
    ...
    def encumbrance(cls, hero: dict):
        ...
        if ActivatablesID.REDUCE_ENC in cls.activatables(hero=hero):
            enc -= cls.activatables(hero=hero)[
                ActivatablesID.REDUCE_ENC][0]['tier']
    ...
    def activatables(cls, hero: dict) -> dict:
        ...
        activatables['ADV'] = {
            key: val for key, val in hero['activatable'].items() if key.startswith('ADV_')}
        activatables['DISADV'] = {
            key: val for key, val in hero['activatable'].items() if key.startswith('DISADV_')}
        activatables['SA'] = {
            key: val for key, val in hero['activatable'].items() if key.startswith('SA_')}
        return activatables
    ...
    def dodge(cls, hero: dict):
        base_dodge = cls.attributes(
            hero=hero, search_for_attr=AttributeID.GE) / 2
    ...
    @classmethod
    def assemble_details(cls, hero: dict()) -> dict:
            ...
            if act_key.startswith('ADV_'):
                ...
                for sa_variation in act_val:
                    ...
                    if len(sa_variation) > 0:
                        ...
                        activatables['SA'][act_key] = SPECIAL_ABILITIES[act_key]
        hero['activatables'] = activatables
